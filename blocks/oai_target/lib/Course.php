...
class Course {
    ...
    function register( $course_id, $starting_time ) {
        ...
        if( isset($CFG->block_update_frequency) ) {
            ...
            $course->update_frequency = 12 * 3600;
        }

        return $DB->insert_record( 'block_oai_target_courses', $course );
    }

    function update_last_update_time( $course_id, $last_update_time ) {
        global $DB;

        $course=new Object();
        $course->id = $this->get_registration_id( $course_id );
        $course->course_id = $course_id;
        $course->last_update_time = $last_update_time;

        return $DB->update_record( 'block_oai_target_courses', $course );
    }

    function update_course_settings( $course_id, $settings ) {
        global $DB;
        ...
        if( isset($settings->update_frequency) ) {
            $course->update_frequency = $settings->update_frequency % 25 * 3600;
        }
    ...
    function get_last_update_time( $course_id ) {
        global $DB;
    ...
    function uses_oai_target_block( $course_id ) {
        global $DB, $CFG;

        $id = $DB->get_records_sql( "select instanceid from {$CFG->prefix}context where id in (select parentcontextid from {$CFG->prefix}block_instances where blockname = 'oai_target') and instanceid = $course_id" );
        if( empty($id) ) {
            return false;
        } else {
            return true;
    ...
    function get_all_courses_using_oai_target_block() {
        global $DB, $CFG;

        // join block_instances, context and course and extract all courses
        // that are using oai_target block
        return $DB->get_records_sql( " select * from {$CFG->prefix}course where id in
                                            ( select instanceid from {$CFG->prefix}context where id in
                                                ( select parentcontextid from {$CFG->prefix}block_instances where blockname = 'oai_target' ) );" );
    }

    function get_updated_and_deleted_modules( $course_id ){
        global $DB;

        $last_update_time = $this->get_last_update_time( $course_id );
        return $DB->get_records_select( 'log', "course=$course_id and action in ('update', 'delete mod') and time > $last_update_time", null,'cmid,action' );
    }
    ...
    function update_log( $course ){
        ...
        foreach($modinfo->cms as $cms => $module) {
            ...
            $DB->insert_record( 'block_oai_target_log', $new_record );
        }
        // update records
    ...
    function initialize_log( $course ){
        ...
        $DB->delete_records( 'block_oai_target_log', array('course_id'=>$course->id)  );
        // add new records
        foreach( $modinfo->cms as $cms => $module ) {
    ...
    function is_module_logged( $course_id, $module_id, $type ){
        ...
        $log = $DB->get_records_select( 'block_oai_target_log', "course_id = $course_id AND module_id = $module_id AND type = '$type'", null,'id' );
        if(empty($log)) {
            return false;
        } else {
            return true;
    ...
    function log_exists( $course_id ){
        ...
        $log = $DB->get_records_select('block_oai_target_log', "course_id = $course_id", null,'id');
        if(empty($log)) {
            return false;
        } else {
            return true;
    ...
    function get_log_entry( $module_id ){
        ...
        $entry = $DB->get_records_select( 'block_oai_target_log', "module_id = $module_id" );
        if ( empty($entry) ) {
            return null;
        } else {
    ...
    function get_logs( $course_id, $limit ){
        global $DB, $CFG;
        $entries = $DB->get_records_sql( "select * from {$CFG->prefix}block_oai_target_log where course_id=$course_id order by id desc limit $limit" );
        if ( empty($entries) ) {
            return null;
        } else {
            return $entries;
    ...
    function get_recent_activities( $course_id ){
        ...
        //block_oai_target_log table plus visible field from course_modules
        $subtable = "( select {$CFG->prefix}block_oai_target_log.*, {$CFG->prefix}course_modules.visible
                        from {$CFG->prefix}block_oai_target_log left join {$CFG->prefix}course_modules
                            on ({$CFG->prefix}block_oai_target_log.module_id = {$CFG->prefix}course_modules.id) ) logs_with_visibility";
        ...
        // clear all pending oai_target
        if(!empty($recent_activities))
            $DB->execute( "update {$CFG->prefix}block_oai_target_log set status = 'notified'
                                where
    ...
    function collect_garbage(){
        ...
        $DB->execute( "delete from {$CFG->prefix}block_oai_target_courses where course_id not in $course_list" );
        $DB->execute( "delete from {$CFG->prefix}block_oai_target_log where course_id not in $course_list" );
    }
